...
import { Predicate, PropPredicate, StatPredicate, PredicateChain, ANDPredicateChain, ORPredicateChain } from '../../models/predicateModels';
...
export class PredicateList extends React.Component<IPredicateListProps, IPredicateListState> {
    ...
    private handleButtonClick(op: string) {
        ...
        if ((isPropPredicate && this.state.eValue === '' && this.state.neValue === '') || // both equal and not equal values are empty
            (!isPropPredicate && Number.isNaN(this.state.lowerBound) && Number.isNaN(this.state.upperBound))) { // both bounds are empty or not numbers
            console.error('you dummy');
            return;
        }
    ...
    public render() {
        ...
        const newFilterForm = (
            <Grid container spacing={2}>
                ...
                <Grid item xs={2}>
                    <TextField id='new-filter-form-equal-input'
                            placeholder='='
                            helperText='Value equal to'
                            value={this.state.eValue}
                            disabled={!isPropPredicate || this.state.neValue !== ''}
                            onChange={(evt) => this.setState({ eValue: evt.target.value as string })} />
                </Grid>
                <Grid item xs={2}>
                    <TextField id='new-filter-form-equal-input'
                                placeholder='≠'
                                helperText='Value not equal to'
                                value={this.state.neValue}
                                disabled={!isPropPredicate || this.state.eValue !== ''}
                                onChange={(evt) => this.setState({ neValue: evt.target.value as string })} />
                </Grid>
                <Grid item xs={2}>
                    <TextField id='new-filter-form-ge-input'
                            placeholder='≥'
                            helperText='Lower bound of stat'
                            type='number'
                ...
                <Grid item xs={2}>
                    <TextField id='new-filter-form-ge-input'
                            placeholder='≤'
                            helperText='Upper bound of stat'
                            type='number'
                ...
                <Grid item>
                    {this.props.filterPredicate === null ? (
                            ...
                            <ButtonGroup>
                                <Button color='primary' onClick={() => this.handleButtonClick('AND')}>
                                    AND
                                </Button>
                                <Button color='secondary' onClick={() => this.handleButtonClick('OR')}>
                                    OR
                                </Button>
                            </ButtonGroup>
                        )}
